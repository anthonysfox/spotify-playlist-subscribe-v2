 ...
 * settings", artboard 7b). Sectioned modal with a sticky diff footer — the
 * subscribe *flow* has its own drawer now (SubscribeSheet), so this surface is
 * only ever edit-with-diff.
 *
 * `7a` puts these same sections on the `/library/[id]/settings` route; that
 * route isn't built yet, so `7b` (the modal) is what ships.
 */
...
export const PlaylistSettingsModal = ({
  ...
  const initial = useRef<Form>({
    syncInterval: playlist.syncInterval ?? "WEEKLY",
    customDays: parseDays(playlist.customDays),
    syncQuantityPerSource: playlist.syncQuantityPerSource || 5,
    syncMode: (playlist.syncMode as "APPEND" | "REPLACE") || "APPEND",
    explicitContentFilter: !!playlist.explicitContentFilter,
    trackAgeLimit: playlist.trackAgeLimit || 0,
  ...
  // Cover art (admin-gated, Spotify-only — same flow as before, restyled).
  const isAdmin =
  ...
  const changes = useMemo(() => {
    const c: string[] = [];
    if (form.syncInterval !== initial.syncInterval) {
      c.push(
        `${freqLabel(initial.syncInterval)} → ${freqLabel(form.syncInterval)}`,
      );
    } else if (
      form.syncInterval === "CUSTOM" &&
      form.customDays.slice().sort().join() !==
        initial.customDays.slice().sort().join()
    ) {
      c.push("Custom days changed");
    }
    if (form.syncQuantityPerSource !== initial.syncQuantityPerSource) {
      c.push(
        `${initial.syncQuantityPerSource} → ${form.syncQuantityPerSource} tracks per source`,
      );
    }
    if (form.syncMode !== initial.syncMode) {
      c.push(
        `${initial.syncMode === "APPEND" ? "Add new songs" : "Replace all"} → ${
          form.syncMode === "APPEND" ? "Add new songs" : "Replace all"
        }`,
      );
    }
    if (form.explicitContentFilter !== initial.explicitContentFilter) {
      c.push(
    ...
    if (form.trackAgeLimit !== initial.trackAgeLimit) {
      c.push(
        `Age limit → ${AGE.find((a) => a.value === form.trackAgeLimit)?.label}`,
      );
    }
    const v0 = initial.vibePrompt.trim();
  ...
  const handleStopManaging = async () => {
    ...
    try {
      ...
      setManagedPlaylists(
        managedPlaylists.filter((p) => p.id !== playlist.id),
      );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
      <div className="flex max-h-[90vh] w-full max-w-[860px] flex-col overflow-hidden rounded-[18px] bg-ground shadow-[0_24px_60px_rgba(26,21,18,0.3)]">
        {/* Header */}
        <div className="flex items-center gap-3.5 border-b border-line bg-surface px-6 py-4">
          {coverUrl ? (
            <img
              src={coverUrl}
              alt=""
            ...
            <span className="art-placeholder h-[52px] w-[52px] shrink-0 rounded-[11px]" />
          )}
          <div className="min-w-0 flex-1">
            <div className="font-mono text-[10.5px] font-medium uppercase tracking-[0.08em] text-ink-35">
              Settings
            </div>
            <div className="truncate font-display text-[19px] font-semibold tracking-[-0.015em] text-ink">
              {playlist.name}
            </div>
            <div className="mt-0.5 text-[12px] text-ink-50">
              {sourceCount} source{sourceCount === 1 ? "" : "s"} ·{" "}
              {playlist.trackCount} tracks · {PROVIDER_LABELS[playlist.provider]}
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex min-h-0 flex-1">
          {/* Section nav */}
          <div className="flex w-[176px] shrink-0 flex-col gap-0.5 border-r border-line bg-surface p-3">
            {SECTIONS.map((s) => (
              <button
                key={s.id}
                type="button"
        ...
        {/* Diff footer */}
        <div className="flex items-center gap-3 border-t border-line bg-surface px-6 py-3.5">
            ...
            <span className="text-[12.5px] leading-snug text-ink-70">
              {dirty ? (
                <>
                  ...
                  {changes.length > 3 ? "…" : ""}. Takes effect from the next
                  run.
                </>
              ) : (
                "No changes yet."
