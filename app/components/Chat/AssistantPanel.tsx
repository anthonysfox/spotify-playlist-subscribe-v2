...
import { MessageBubble, TypingDots } from "./ChatMessage";
...
/**
 * The "ask the fox" drawer (README "Screens" > AI assistant). A right-docked
 * panel — same slot language as the subscribe sheet.
 *
 * Loaded lazily (see SearchAssistant): `@ai-sdk/react` + the markdown renderer
 * only enter the bundle once the user opens the assistant for the first time.
 * Kept mounted after that so the conversation persists; visibility toggles with
 * transform only. `inert` while closed keeps its contents out of tab order.
 *
