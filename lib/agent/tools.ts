import { tool } from "ai";
import z from "zod";
import { getProvider, MusicProvider } from "../music";
import prisma from "../prisma";
import type { AgentProposal } from "./proposals";

const FREQ_WORD: Record<string, string> = {
  DAILY: "daily",
  WEEKLY: "weekly",
  MONTHLY: "monthly",
};

export function buildTools(userId: string) {
  return {
    searchPlaylists: tool({
      description: "Search a music service for playlists to subscribe to.",
      inputSchema: z.object({
        query: z.string(),
        provider: z.enum(["SPOTIFY", "APPLE_MUSIC"]).default("SPOTIFY"),
      }),
      execute: async ({ query, provider }) => {
        const client = await getProvider(provider as MusicProvider).forUser(
          userId,
        );
        if (!client) return { error: `${provider} isn't connected` };
        return client.searchPlaylists(query, 10);
      },
    }),

    listManagedPlaylistDetails: tool({
      description:
        "Lists the details of a managed playlist including its settings and source playlists. Don't include anything about IDs in the response for either the managed playlist or the source playlists",
      inputSchema: z.object({
        managedPlaylistId: z.string(),
      }),
      execute: async ({ managedPlaylistId }) => {
        const playlist = await prisma.managedPlaylist.findFirst({
          where: { userId, deletedAt: null, id: managedPlaylistId },
          include: {
            subscriptions: {
              where: {
                sourcePlaylist: {
                  deletedAt: null,
                },
              },
              include: {
                sourcePlaylist: true,
              },
            },
          },
        });
        if (!playlist) return { error: "Managed playlist not found." };

        // Flatten to plain, JSON-serializable fields — the raw Prisma record
        // carries native Date objects (createdAt, nextSyncTime, ...) at every
        // level, and the AI SDK can't fold those into the next model call's
        // prompt (see the same note on createSubscription's return above).
        return {
          name: playlist.name,
          provider: playlist.provider,
          trackCount: playlist.trackCount,
          syncInterval: playlist.syncInterval,
          syncQuantityPerSource: playlist.syncQuantityPerSource,
          syncMode: playlist.syncMode,
          explicitContentFilter: playlist.explicitContentFilter,
          trackAgeLimit: playlist.trackAgeLimit,
          vibePrompt: playlist.vibePrompt,
          lastSyncCompletedAt:
            playlist.lastSyncCompletedAt?.toISOString() ?? null,
          nextSyncTime: playlist.nextSyncTime?.toISOString() ?? null,
          sources: playlist.subscriptions.map((sub) => ({
            sourcePlaylistId: sub.sourcePlaylist.id,
            name: sub.sourcePlaylist.name,
            trackCount: sub.sourcePlaylist.trackCount,
          })),
        };
      },
    }),

    generatePlaylist: tool({
      description:
        "Create a brand-new playlist of songs that match a described vibe or " +
        "mood (NOT sourced from existing playlists). Use this when the user wants " +
        "a playlist built from a description like 'chill rainy-day indie'. " +
        "One-shot — the result is a normal playlist, not auto-synced. Returns a " +
        "proposal the user confirms before anything is created.",
      inputSchema: z.object({
        vibe: z
          .string()
          .describe("The mood/genre/vibe, e.g. 'late-night synthwave drive'"),
        playlistName: z.string().describe("Name for the new playlist"),
        count: z.number().int().min(5).max(40).default(20),
        provider: z.enum(["SPOTIFY", "APPLE_MUSIC"]).default("SPOTIFY"),
      }),
      execute: async ({ vibe, playlistName, count, provider }) => {
        const client = await getProvider(provider).forUser(userId);
        if (!client) return { error: `${provider} isn't connected` };

        const proposal: AgentProposal = {
          action: "generatePlaylist",
          title: `Create “${playlistName}”`,
          detail: `Build a new ${count}-track ${provider === "APPLE_MUSIC" ? "Apple Music" : "Spotify"} playlist for “${vibe}”. It's a one-off — not auto-synced.`,
          params: { provider, vibe, playlistName, count },
        };
        return { proposal };
      },
    }),

    listManagedPlaylists: tool({
      description: "List the user's managed playlists",
      inputSchema: z.object({}),
      execute: async () =>
        prisma.managedPlaylist.findMany({
          where: { userId, deletedAt: null },
          select: { id: true, name: true, provider: true },
        }),
    }),

    addArtistsToPlaylist: tool({
      description:
        "Add more music from specific artists into an EXISTING managed playlist " +
        "on an ongoing basis. Use this when the user says things like 'add more " +
        "John Mayer' or 'I want more Justin Bieber and Jon Bellion in my Chill " +
        "playlist'. For each artist, it finds the service's official artist " +
        "playlist (e.g. Spotify's 'This Is <Artist>', which blends the artist's " +
        "catalog with similar songs) and subscribes the destination playlist to " +
        "it — so it syncs in fresh tracks now AND keeps adding more of that " +
        "artist on the normal sync schedule, not just once. Requires an " +
        "existing managedPlaylistId (look it up with listManagedPlaylists first, " +
        "or ask the user which playlist). Returns a proposal the user confirms.",
      inputSchema: z.object({
        managedPlaylistId: z.string(),
        artists: z
          .array(z.string())
          .min(1)
          .max(5)
          .describe("Artist names, e.g. ['John Mayer', 'Justin Bieber']"),
        syncFrequency: z.enum(["DAILY", "WEEKLY", "MONTHLY"]).default("WEEKLY"),
        provider: z.enum(["SPOTIFY", "APPLE_MUSIC"]).default("SPOTIFY"),
      }),
      execute: async ({
        managedPlaylistId,
        artists,
        syncFrequency,
        provider,
      }) => {
        const dest = await prisma.managedPlaylist.findFirst({
          where: { id: managedPlaylistId, userId, deletedAt: null },
        });
        if (!dest) return { error: "Destination playlist not found." };

        const proposal: AgentProposal = {
          action: "addArtists",
          title: `Add ${artists.join(", ")} to ${dest.name}`,
          detail: `Subscribe ${dest.name} to the official mix${artists.length > 1 ? "es" : ""} for ${artists.join(", ")}, syncing ${FREQ_WORD[syncFrequency] ?? "weekly"}. Fresh tracks now, and more on every sync.`,
          params: {
            provider: provider as MusicProvider,
            managed: {
              id: dest.externalPlaylistId,
              name: dest.name,
              imageUrl: dest.imageUrl,
              trackCount: dest.trackCount,
            },
            artists,
            syncFrequency,
          },
        };
        return { proposal };
      },
    }),

    createSubscription: tool({
      description:
        "Subscribe a source playlist to a managed one. Returns a proposal the " +
        "user confirms in the UI before anything is created — do not assume it " +
        "succeeded.",
      inputSchema: z.object({
        sourcePlaylistId: z.string(),
        newPlaylistName: z.string().optional(),
        managedPlaylistId: z.string().optional(),
        syncFrequency: z.enum(["DAILY", "WEEKLY", "MONTHLY"]).default("WEEKLY"),
        provider: z.enum(["SPOTIFY", "APPLE_MUSIC"]).default("SPOTIFY"),
      }),
      execute: async ({
        sourcePlaylistId,
        newPlaylistName,
        managedPlaylistId,
        syncFrequency,
        provider,
      }) => {
        const client = await getProvider(provider).forUser(userId);
        if (!client) return { error: `${provider} isn't connected` };

        const source = await client.getPlaylist(sourcePlaylistId);
        if (!source) return { error: "Source playlist not found." };

        let managedPlaylist;
        let destName = newPlaylistName ?? "a new playlist";
        if (managedPlaylistId) {
          const dest = await prisma.managedPlaylist.findFirst({
            where: { id: managedPlaylistId, userId, deletedAt: null },
          });
          if (!dest) return { error: "Destination playlist not found." };
          managedPlaylist = {
            id: dest.externalPlaylistId,
            name: dest.name,
            imageUrl: dest.imageUrl,
            trackCount: dest.trackCount,
          };
          destName = dest.name;
        }

        const proposal: AgentProposal = {
          action: "subscribe",
          title: `Subscribe to ${source.name}`,
          detail: `New tracks from ${source.name} get added to ${destName} ${FREQ_WORD[syncFrequency] ?? "weekly"}.${managedPlaylist ? "" : " That makes it a managed playlist."}`,
          params: {
            provider: provider as MusicProvider,
            sourcePlaylist: {
              id: source.id,
              name: source.name,
              imageUrl: source.imageUrl,
              trackCount: source.trackCount,
            },
            managedPlaylist,
            newPlaylistName,
            syncFrequency,
          },
        };
        return { proposal };
      },
    }),

    removeSource: tool({
      description:
        "Unsubscribe a source playlist from a managed playlist, so it stops " +
        "pulling songs from it. Get the sourcePlaylistId from " +
        "listManagedPlaylistDetails. If this is the playlist's last source, " +
        "the managed playlist is removed from PlaylistFox entirely — the " +
        "playlist itself isn't touched on the music service, it just stops " +
        "being auto-synced. Returns a proposal the user confirms.",
      inputSchema: z.object({
        managedPlaylistId: z.string(),
        sourcePlaylistId: z.string(),
      }),
      execute: async ({ managedPlaylistId, sourcePlaylistId }) => {
        const playlist = await prisma.managedPlaylist.findFirst({
          where: { id: managedPlaylistId, userId, deletedAt: null },
          include: {
            subscriptions: {
              where: { sourcePlaylist: { deletedAt: null } },
              include: { sourcePlaylist: { select: { id: true, name: true } } },
            },
          },
        });
        if (!playlist) return { error: "Playlist not found." };
        const sub = playlist.subscriptions.find(
          (s) => s.sourcePlaylist.id === sourcePlaylistId,
        );
        if (!sub) return { error: "That source isn't on this playlist." };

        const lastSource = playlist.subscriptions.length === 1;
        const proposal: AgentProposal = {
          action: "unsubscribe",
          title: `Remove ${sub.sourcePlaylist.name}`,
          detail: lastSource
            ? `${sub.sourcePlaylist.name} is the only source, so ${playlist.name} stops being managed. The playlist and its tracks stay in your account.`
            : `${playlist.name} stops pulling tracks from ${sub.sourcePlaylist.name}.`,
          params: { managedPlaylistId, sourcePlaylistId, lastSource },
        };
        return { proposal };
      },
    }),
  };
}
